...
[English:README](README.md) | [イマココ!](README.ja.md) | [한국어:README](README.ko.md) | [繁體中文:README](README.zh-tw.md) | [简体中文:README](README.zh-cn.md)
...
- **[NEW]** チャットタイムスタンプ (チャットに `[HH:MM:SS]` を表示可能に)
- 不適切な単語のフィルタリング (現在メンテナンス中)
- クエストのピン留め
- 経験値トラッカー（1分あたりの経験値と次のレベルまでの推定時間）
- **[NEW]** ゴールドとエリーフの獲得量トラッカー
- **[NEW]** 日付変更までの時間を表示
- **[NEW]** 低体力アラート (画面端が赤く点滅する)
- **[NEW]** HUDカスタマイズ: UI拡大縮小スライダー / ガラスモーフィズム / 不透明度スライダー / アクセントカラー選択
- **[NEW]** HUD背景に画像を表示
...
- **[NEW]** ドローンモード / フリーカメラモード (シネマティックな撮影が可能。※ゲームパッド専用)
...
- **[NEW]** チャット送信自動翻訳コマンド（`.j`/`.ja`/`.jp <本文>`, `.k`/`.ko <本文>`, `.zh`/`.cn <本文>`, `.en <本文>`, `.es <本文>` 等で指定言語に自動翻訳して発言）
