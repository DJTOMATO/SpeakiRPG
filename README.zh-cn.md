...
* **自定义聊天表情**: 这是一个完全交互式的自定义表情选择面板，为你最喜欢的 Discord 风格表情提供巨大的悬停预览。
...
- **[NEW]** 聊天发送自动翻译指令（`.j`/`.ja`/`.jp <内容>`, `.k`/`.ko <内容>`, `.zh`/`.cn <内容>`, `.en <内容>`, `.es <内容>` 等自动翻译为指定语言并送出）
...
- **[NEW]** 官方更新公告简报 (点击 📰 按钮在游戏内查看自动翻译的更新说明)
- **[NEW]** 本次游玩与性能统计窗口 (点击 ⏱️ 按钮实时查看游玩时长、每小时EXP、升级所需时间、金币/水晶叶收益与网络延迟)
- **[NEW]** 快速登录与账号管理器 (点击 HUD 的 🔑 按钮或按 F2 键开启。支持恢复代码安全保存、查看与复制；仅本地保存绝不外流)
- **[NEW]** 更新与最新消息通知窗口 (在游戏内实时获取客户端最新版本更新通知与日志)
