...
* **정확한 퀘스트 잠금**
\n
* **커스텀 채팅 이모티콘**: 좋아하는 디스코드 스타일의 이모티콘을 위한 큰 호버 미리보기가 있는 완벽하게 대화형인 커스텀 이모티콘 선택 패널
...
- 의식 댄스 (점프 및 하트 이모트와 연동되는 원형 궤도 댄스, 역방향 모드 추가)
...
- **[NEW]** 채팅 발신 자동 번역 명령어 (`.j`/`.ja`/`.jp <내용>`, `.k`/`.ko <내용>`, `.zh`/`.cn <내용>`, `.en <내용>`, `.es <내용>` 등으로 해당 언어로 번역 후 전송)
...
- **[NEW]** 공식 패치노트 뉴스레터 (📰 버튼으로 인게임에서 번역된 게임 업데이트 내역 확인)
- **[NEW]** 세션 및 성능 통계 창 (⏱️ 버튼으로 세션 시간, 시간당 EXP, 레벨업까지, 골드/엘리프 획득량, 네트워크 핑 확인)
- **[NEW]** 간편 로그인 및 계정 관리자 (HUD의 🔑 버튼 또는 F2 키로 열기. 복구 코드 로컬 저장·확인·복사 지원; 기기 로컬에만 안전하게 저장되며 외부 공유 없음)
- **[NEW]** 업데이트 및 소식 알림 창 (클라이언트 최신 버전 알림 및 패치 소식을 인게임에서 바로 확인)
